...
const telemetryEndpoint = config.telemetryEndpoint || "telemetryAts.php?format=json";
...
const defaultTileBaseUrlCandidates = [
    "https://framedev.ch/tilesMaps/atsTiles/",
];
const defaultTileConfigNames = ["TileMapInfo.json"];
...
function formatTruckSpeed(speedKph) {
    ...
    const mph = Math.max(0, parsed) * kphToMph;
    return formatNumber(mph, mph < 10 ? 1 : 0);
}
...
function truckLimitNote(truck = {}) {
    return truck.cruiseControlOn
        ? `Cruise ${formatTruckSpeed(truck.cruiseControlSpeed)} mph`
...
function renderHero(data) {
    ...
    if (elements.speedPeak) {
        const speedRingPeakMph = speedRingPeakKph * kphToMph;
        elements.speedPeak.textContent = `Peak ${formatNumber(speedRingPeakMph, speedRingPeakMph < 10 ? 1 : 0)} mph`;
    }

    if (elements.speedTrend) {
        const speedDeltaMph = Math.abs(speedDelta) * kphToMph;
        const trendLabel = trendState === "rising"
            ? `Rising +${formatNumber(speedDeltaMph, 1)}`
            : trendState === "falling"
                ? `Falling -${formatNumber(speedDeltaMph, 1)}`
                : "Steady";
    ...
    if (elements.speedAlert) {
        elements.speedAlert.textContent = isOverspeed
            ? `Overspeed +${formatNumber(Math.max(0, speedKph - (roadLimitKph ?? speedKph)) * kphToMph, 1)} mph`
            : "";
    ...
    if (elements.heroTags) {
        ...
        if (truck.cruiseControlOn === true) {
            heroTagItems.push(createStatePill(`Cruise ${formatTruckSpeed(truck.cruiseControlSpeed)} mph`, true));
        }
...
function renderRoute(data) {
    ...
    if (elements.routeStats) {
        elements.routeStats.innerHTML = [
            ...
            createStatCard("Speed limit", navigation.speedLimit ? `${formatRoadSpeed(navigation.speedLimit)} mph` : "--", truckLimitNote(data.truck)),
...
function renderDrivetrain(data) {
    ...
    if (elements.drivetrainStats) {
        elements.drivetrainStats.innerHTML = [
            ...
            createStatCard("Cruise", truck.cruiseControlOn ? `${formatTruckSpeed(truck.cruiseControlSpeed)} mph` : "--", boolLabel(truck.cruiseControlOn, "Holding speed", "Cruise inactive")),
...
function renderAlerts(data) {
    ...
    if (isAlertGroupEnabled("overspeed") && hasDangerousOverspeed) {
        alerts.push(createAlertItem(
            "Overspeed",
            `Limit ${formatRoadSpeed(roadLimitKph)} mph • Current ${formatTruckSpeed(truck.speed)} mph`,
...
function renderWorld(data) {
    ...
    if (elements.worldStats) {
        elements.worldStats.innerHTML = [
            ...
            createStatCard("Navigation", formatDistanceKm(getNavigationDistanceKm(navigation)), navigation.speedLimit ? `Speed limit ${formatRoadSpeed(navigation.speedLimit)} mph` : "No limit data"),
        ].join("");
    }
}
